...
class UNetGeneratorDeep(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, features=64):
        ...
        # Zaustavite se na 2x2, nema down8 i down9
        
        # Dodajte residual blok za dubinu
        ...
        # U decoder dijelu je jedna up operacija manje
...
# Definiranje ResidualBlock-a za diskriminator koji koristimo gore
...
class VGG19FeatureExtractor(nn.Module):
    """VGG19 Feature Extraktor koji će služiti kao encoder u generatoru"""
    def __init__(self, requires_grad=False):
        ...
        # Zamrznimo parametre ako ne želimo trenirati VGG
        if not requires_grad:
            for param in self.vgg.parameters():
                param.requires_grad = False
                    
    def forward(self, x):
        # Sačuvamo međurezultate za skip konekcije
        features = []
        
        # Prolazak kroz slojeve VGG19
        for i, layer in enumerate(self.vgg):
            x = layer(x)  # Primjenjujemo sloj na tenzor x
...
class VGG19Generator(nn.Module):
    """Generator baziran na VGG19 za CycleGAN s ispravnim dimenzijama za 256x256 ulaz"""
    def __init__(self, in_channels=1, out_channels=1, features=64):
        super(VGG19Generator, self).__init__()
        
        # Adapter za grayscale ulaz -> RGB za VGG19
        ...
        # Pomoćni slojevi za prilagodbu broja kanala iz VGG19 značajki
        ...
        # Decoder: Transponirane konvolucije s prilagođenom strukturom za VGG19
        # Početna veličina je 512 kanala na najmanjoj prostornoj rezoluciji
        self.upconv1 = nn.ConvTranspose2d(512, 256, kernel_size=4, stride=2, padding=1)
        self.norm1 = nn.InstanceNorm2d(256)
        
        # Nakon upconv1 i concat s adapt_256 imamo 256+256=512 kanala
        self.upconv2 = nn.ConvTranspose2d(512, 128, kernel_size=4, stride=2, padding=1)
        self.norm2 = nn.InstanceNorm2d(128)
        
        # Nakon upconv2 i concat s adapt_128 imamo 128+128=256 kanala
        self.upconv3 = nn.ConvTranspose2d(256, 64, kernel_size=4, stride=2, padding=1)
        self.norm3 = nn.InstanceNorm2d(64)
        
        # Nakon upconv3 i concat s adapt_64 imamo 64+64=128 kanala na 256x256
        # IZMJENA: umjesto upconv4 koristimo običnu konvoluciju koja ne mijenja dimenzije
    ...
    def forward(self, x):
        # Prilagodba za VGG19 (1 kanal -> 3 kanala)
        x_rgb = self.input_adapter(x)
        
        # Izvuci značajke iz VGG19
        vgg_features = self.encoder(x_rgb)
        ...
        # Prilagodimo broj kanala ako je potrebno
        ...
        # IZMJENA: Običan konvolucijski sloj umjesto upconv4
